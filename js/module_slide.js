const slideShow = (target) => {
  let slideWp = document.querySelector(target);
  let slidecon = slideWp.querySelector(".slide-container");
  let slide = slidecon.querySelectorAll("li");
  let page = slideWp.querySelector(".pager");
  let slideCount = slide.length;
  let currnetIndex = 0;
  let timer;
  let navPrev = slideWp.querySelector(".prev");
  let navNext = slideWp.querySelector(".next");
  console.log(slide);
  if (slideCount > 0) {
